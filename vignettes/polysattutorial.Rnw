...
\url{http://www.r-project.org/}.  Once R is installed, polysat can be
...
In the ``extdata'' directory of the polysat installation there are
...
polysat 1.2 and is called \texttt{meandistance.matrix2}.  Whereas
