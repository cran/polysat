% \VignetteIndexEntry{polysat version 1.0 Tutorial Manual}
...
\title{polysat version 1.0 Tutorial Manual}
...
\ref{sec:popstat} for important information about allele
...
Three other population genetic programs that I am aware of can handle
...
polysomic inheritance (autopolyploidy): Structure \cite{falush07,
  falush03, pritchard00, hubisz09}, SPAGeDi \cite{hardy02}, and
...
files called ``structureExample.txt'', ``spagediExample.txt'', and
``genodiveExample.txt''.  To import these into \texttt{"genambig"}
...
\texttt{"genambig"} objects that you have just created.  All three of
these functions take population information from the file and put it
into the object.  The Structure and SPAGeDi files are coded in a way
...
Autopolyploid data can also be exported in the same five formats that are
...
sections \ref{sec:gsinddist} and \ref{sec:inddist}.
...
We are revising an article for Molecular Ecology Resources:
