% \VignetteIndexEntry{polysat version 1.4 Tutorial Manual}
...
\title{\textsc{polysat} version 1.4 Tutorial Manual}
...
vignette ``Assigning alleles to isoloci in \textsc{polysat}'' to learn
more about how to determine which alleles belong to which genomes.
