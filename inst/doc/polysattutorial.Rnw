% \VignetteIndexEntry{polysat version 1.3 Tutorial Manual}
...
\title{polysat version 1.3 Tutorial Manual}
...
locus (\emph{e.g} if some of your loci are on sex chromosomes).  The object in
...
pair with each other at meiosis, many of the analyses in polysat that utilize
ploidy do not apply.
...
polysat also supports three genotype formats that work for either
...
format uses one row 
per individual, the modified format for \texttt{polysat} includes a column to
