% \VignetteIndexEntry{polysat version 1.2 Tutorial Manual}
...
\title{polysat version 1.2 Tutorial Manual}
...
The R package \texttt{polysat} provides useful tools for
...
Structure, and POPDIST.  It can also calculate pairwise genetic distances
between
...
\url{http://www.r-project.org/}.  Once installed, polysat can be
...
\texttt{polysat}.  The first steps will always be importing or
...
\node at (0, -3) [block] (binary) {Convert to presence/absence
...
\node at (1.9, -9.9) [block] (ploidies) {Add ploidy info with
  \texttt{Ploidies} function.};
...
\node at (0, -15.5) [block] (alloexport) {Export tetraploid data to
...
\path [line] (allo) |- node[below] {yes} (alloexport);
...
\node at (2.5, -12) [block] (calcfst) {Get distances between
  populations with \texttt{calcFst}.};
...
<<echo=false>>=
if(interactive()) simgen <- editGenotypes(simgen, maxalleles = 4)
@
...
<<echo=false>>=
testmat <- as.matrix(read.table("vignettebuild/testmat.txt"))
@
...
<<echo=false>>=
testmat2 <- as.matrix(read.table("vignettebuild/testmat2.txt"))
@
...
for another method of calculating inter-individual distances.
...
polysat.  The \texttt{deleteSamples} and \texttt{deleteLoci} functions
...
\section{How data are stored in polysat}
...
polysat uses the S4 class system in R.  ``Class'' and ``object'' are
...
rather than nucleotides), \texttt{Ploidies} (a vector containing the
ploidy of each sample, or \texttt{NA} if unknown), \texttt{PopNames}
...
The \texttt{"genbinary"} class is also used by \texttt{polysat} to
...
The class system in polysat is set up so that anyone can extend
...
In order to properly utilize polysat (and other software for polyploid
data) it is important to understand the inheritance mode in your
system.  In an autopolyploid, all homologous chromosomes are equally
...
In the ``doc'' directory of the polysat installation there are
...
these functions take population information from the file and put it
...
polysat also supports two genotype formats that work for either
autopolyploids or allopolyploids, but do not contain any population,
ploidy, or other information: GeneMapper, and binary
...
The example file in the ``doc'' directory can be read in the
following way:
...
A few functions in polysat will work directly on a
...
Autopolyploid data can also be exported in the same six formats that are
...
exporting allele frequencies from polysat to SPAGeDi for use in these
calculations.
...
used.  The two options that are provided with polysat are
\texttt{Bruvo.distance}, which takes mutational distance between
...
too computationally intensive (see \texttt{?Bruvo.distance}).

A second means of calculating inter-individual distances is provided
with polysat 1.2 and is called \texttt{meandistance.matrix2}.  Whereas
...
\texttt{Bruvo.distance} or a user-defined distance measure.
...
<<echo=FALSE>>=
testmat4 <- as.matrix(read.table("vignettebuild/testmat4.txt"))
@
<<echo=TRUE, fig=TRUE>>=
...
Very similarly to the software GenoType \cite{meirmans04}, polysat can use a
...
There are two functions in polysat for estimating allele
...
tables produced by \texttt{polysat} can be converted to a format that
...
\cite{simpson49}, are provided with polysat.  The function
...
frequencies.  \texttt{genotypeDiversity} uses the function
...
described in the previous section are appropriate.

Otherwise, the following functionality is available for allopolyploids
in polysat:
...
using polysat.  These programs are able to resolve ambiguity about how
alleles are distributed across the two diploid genomes in an
allotetraploid.  From there, the programs can calculate allele
...
The \texttt{estimatePloidy} function works equally well on
...
Clark, LV and Jasieniuk, M.  {\sc polysat}: an R package for polyploid
microsatellite analysis.  \emph{Molecular Ecology Resources}
11(3):562--566.

Feel free to email me at lvclark@ucdavis.edu with any questions,
